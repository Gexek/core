...
(function($) {
    $.widget("ui.tagit", {
        ...
        _create: function() {
            ...
            this.options.select = function(event, ui) {
                ...
                if (self.options.maxTags !== undefined && self.tagsArray.length == self.options.maxTags) {
                    ...
                    if (ui.item.label === undefined) self._addTag(ui.item.value);
                    else self._addTag(ui.item.label, ui.item.value);
                }

                return false;
            },

            this.options.focus = function(event, ui) {//alert("here");
                if (ui.item.label !== undefined && /^menufocus/.test(event.originalEvent.type)) {
            ...
            if (this.options.select) {
                this.select = $('<select class="tagit-hiddenSelect" name="' + (this.element.attr('name') || this.element.data('name')) + '" id="' + (this.element.attr('id') || this.element.data('id')) + '_select" multiple="multiple"></select>');
        ...
        _processKeyEvent: function(e) {
            ...
            if (lastLi.hasClass('selected')) lastLi.removeClass('selected');
        ...
        _popTag: function(tag) {
            ...
            if (tag === undefined) tag = this.tagsArray.pop();
            else
            ...
            if (this.options.select) this._popSelect(tag);
            if (this.options.tagsChanged) this.options.tagsChanged(tag ? (tag.value || tag.label) : null, 'popped', tag);
            return;
        },
        ...
        _addTag: function(label, value) {
            ...
            tag.element = $('<li class="tagit-choice"' + (value !== undefined ? ' tagValue="' + value + '"' : '') + '>' + (this.options.sortable == 'handle' ? '<a class="ui-icon ui-icon-grip-dotted-vertical" style="float:left"></a>' : '') + '<div class="tagit-label">' + label + '</div>' + '<a class="tagit-close">x</a></li>');
            tag.element.insertBefore(this.input.parent());
            this.tagsArray.push(tag);

            this.input.val("");

            if (this.options.select) this._addSelect(tag);
            if (this.options.tagsChanged) this.options.tagsChanged(tag.label, 'added', tag.element);
            return true;
        },
        ...
        _exists: function(label, value) {
            ...
            for (var ind in this.tagsArray) {
                if (this._lowerIfCaseInsensitive(this.tagsArray[ind].label) == label) {
                    if (value !== undefined) {
                        if (this._lowerIfCaseInsensitive(this.tagsArray[ind].value) == value) return ind;
                    } else {
                        return ind;
                    }
                }
        ...
        _highlightExisting: function(index) {
            if (this.options.highlightOnExistColor === undefined) return;
        ...
        _isInitKey: function(keyCode) {
            ...
            if ($.inArray(keyCode, this._keys[key]) != -1) keyName = key;

            if ($.inArray(keyName, this.options.triggerKeys) != -1) return true;
            return false;
        },
        ...
        _initialTags: function() {
            var input = this;
            var _temp;
            if (this.options.tagsChanged) _temp = this.options.tagsChanged;
            this.options.tagsChanged = null;

            if (this.options.initialTags.length != 0) {
                $(this.options.initialTags).each(function(i, element) {
                    if (typeof(element) == "object") input._addTag(element.label, element.value);
                    else
        ...
        _lowerIfCaseInsensitive: function(inp) {
            ...
            if (inp === undefined || typeof(inp) != typeof("a")) return inp;

            if (this.options.caseSensitive) return inp;
        ...
        reset: function() {
            ...
            this._initialTags();
            if (this.options.tagsChanged) this.options.tagsChanged(null, 'reset', null);
        },

        fill: function(tags) {

            if (tags !== undefined) this.options.initialTags = tags;
            this.reset();
        },

        add: function(label, value) {
            if (typeof(label) == "object") return this._addTag(label.label, label.value);
            else
